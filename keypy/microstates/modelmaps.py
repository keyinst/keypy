...
def get_io_modelmap_for_series(series, iteration, inputfolder, outputfolder, first_input):
    """
...
def find_model_maps(confobj, model_maps_foundation):
    ...
    while iii < seed_number:
        ...
        while (ii < max_number_of_iterations) and abs(delta_correlation) > 0.0002:
            ...
            #set GFP of randmap to 1            
            for ri in range( randmap.shape[0] ):    
                # compute normalization_factor for randmap
                normalization_factor_for_randmap = compute_gfp(randmap[ri,:].reshape( (1,randmap.shape[1]) ))[0]
...
def run_model_maps_series(series, inputfolder, outputfolder, first_input, confobj):
    """
    ...
    iteration = 0
    while True:
        inputhdf5, outputhdf5, modelmap_input, modelmap_output, computation_version, stop = get_io_modelmap_for_series(series, iteration, inputfolder, outputfolder, first_input)
        if stop:
            break
