...
###############################################################
# test5_mstates.py computes a series of modelmaps at a time ###
###############################################################
...
class Test_test5_mstates(unittest.TestCase):
    def test5_mstates(self):
        ################################
        ...
        inputfolder = os.path.join(library_path, "..\\data\\test5")
        ...
        for series in series_versions:
            ...
            outputfolder = os.path.join(library_path, "..\\data\\test5_output\\{0}" .format(series))
            if not os.path.exists(outputfolder):
                os.makedirs(outputfolder)
